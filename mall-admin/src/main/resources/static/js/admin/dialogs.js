        ...
        $.ajax({
            ...
            success: function (ret) {
                ...
                if(200 == ret.code){
                    ...
                    layer.msg(ret.message||'信息保存失败', {icon: 1, time: 1000});
                }
            },
...
function enableFun(data,enable) {
    delFunction(data,'您确定要'+ enable +'该数据吗？');
}

function delFun(data) {
    delFunction(data,'您确定要删除该数据吗？');
}
function delFunction(data,title){
   $("body").css("height","100%");
    layer.confirm( title, {
        btn: ['确定','取消'] //按钮
    }, function(){
        $.ajax({
            url: (data && typeof data === "string") ? data : data.url,
            type: "get",
            dataType: "json",
            success: function (ret) {
                if(200 == ret.code) {
                    layer.msg((ret.msg?ret.msg:"操作成功！"),{icon : 1, time : 1000});
                    if(data.delOkFun && $.isFunction(data.delOkFun)){
                        data.delOkFun(ret);//执行删除成功后的回调
                    }
                }else{
                    layer.msg((ret.message?ret.message:"操作失败，请稍后再试"),{icon : 1, time : 1000});
                }
            },
            error: function (req, status, e) {
                layer.msg("操作失败，请稍后再试",{icon : 5, time : 1000});
                if(data.delFailFun && $.isFunction(data.delFailFun)){
                    data.delFailFun(req, status, e);//执行删除成功后的回调
                }
            }
        });
    }, function(){//取消
